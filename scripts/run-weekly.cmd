...
REM
REM Guard: run at most once per week, anchored to SATURDAY. We skip if a run has
REM already happened on or after the most recent Saturday. This is drift-free and
...
REM Gate: RUN unless the last successful run was already on/after this week's Saturday.
for /f "usebackq delims=" %%g in (`powershell -NoProfile -Command "$s='%STAMP%'; $f=(Get-Date).Date; while($f.DayOfWeek -ne 'Saturday'){$f=$f.AddDays(-1)}; if(Test-Path $s){try{$l=[datetime]::ParseExact((Get-Content $s -Raw).Trim(),'yyyy-MM-dd',$null)}catch{$l=[datetime]::MinValue}}else{$l=[datetime]::MinValue}; if($l -ge $f){'SKIP'}else{'RUN'}"`) do set "GATE=%%g"

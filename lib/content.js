...
// Cache key/tag for the posts list. Bust with revalidateTag(POSTS_TAG) on any
// write (publish/edit/unpublish/discard) so changes appear immediately.
...
// Raw fetch from the active backend (one DB query / one directory read).
...
// Cross-request data cache: one cached posts list shared by every page/route,
// refreshed at most every 60s or immediately when POSTS_TAG is revalidated.
const readAllPersistent = unstable_cache(readAllRaw, ["posts-all"], {
  tags: [POSTS_TAG],
  revalidate: 60,
});

// Per-request dedup: multiple calls in a single render (e.g. getBySlug +
// getByCategory + generateMetadata) collapse to one underlying read.
const readAll = cache(readAllPersistent);
